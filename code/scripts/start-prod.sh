...
docker compose down
docker compose -f docker-compose.yml -f docker-compose-prod.yml --project-name osi-chess up -d
...
docker exec "${CONTAINER_NAME}" /bin/bash -c "export XDEBUG_MODE=coverage && /var/www/vendor/bin/pest --coverage-clover tests/Coverage/Pest/result.xml"
#echo "Running unit tests with Jest"
#docker exec "${CONTAINER_NAME}" npm run coverage
